    ...
    <div className='flex flex-col w-screen h-screen justify-start items-center bg-homepage-background bg-cover p-6 gap-6'>
      {
        quizId.map((element: string) => {
          return <Quiz subjectFolder={"admin"} coverageName={params.coverageName} mode={"coverage"} topicName={params.topicName} difficulty={difficulty} quizId={element} key={element} isAdmin={true}/>
        })
      }
