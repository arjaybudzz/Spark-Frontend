...
const Post = (props: {[key: string]: any}) => {
    ...
    const updatePost = async(data: string): Promise<void> => {
        const url = `http://127.0.0.1:3001/api/v1/posts/${props.postId}`;
        await axios.put(url, {
            body: data
    ...
    const storePostToken = async(): Promise<void> => {
        const url = `http://127.0.0.1:3001/api/v1/post_tokens?post[body]=${props.body}`;
    ...
    const getOwner = async(): Promise<void> => {
        const url = `http://127.0.0.1:3001/api/v1/users/${props.userId}`;
